...
- Hardened `opentt_matches_list` `highlight="auto"` fallback on single-club pages: when league cannot be resolved directly for a season variant, it now seeds league/season from the latest club row and keeps `club_id` filtering until a concrete league is found (prevents season-wide cross-league leakage).
- Added a second single-club season fallback for `opentt_matches_list`: when direct SQL season matching returns no rows, shortcode now loads club rows and applies normalized league/season filtering in PHP so `highlight="auto"` works with mixed season slug formats.
- Fixed `opentt_matches_list` empty-state edge case where `highlight="auto"` had already dropped `club_id`: fallback recovery now restores the single-club context from auto-highlight IDs, widens seed query scope, and retries season-only normalization when league pinning is stale.
- Fixed `opentt_matches_list` season-selection semantics on single-club pages: season chosen via `?opentt_sezona=` is now treated as explicit season input (same as shortcode `season/sezona` attrs), preventing accidental "latest competition" re-scoping.
- Added default single-club season bootstrap for `opentt_matches_list`: when no `?opentt_sezona=` (or shortcode season attr) is present, the shortcode now auto-resolves the club's latest season from its newest match row and renders immediately.
- Added matching default-season behavior to `opentt_players` on single-club pages: without `?opentt_sezona=`, the shortcode now auto-selects the club's latest available season (from `db_get_club_season_options`) instead of showing players aggregated from all seasons.
