...
require_once __DIR__ . '/modules/trait-opentt-unified-search.php';
...
final class OpenTT_Unified_Core
{
    ...
    use OpenTT_Unified_Search_Trait;
    ...
    {
        \OpenTT\Unified\WordPress\FrontendSearchController::handle([
            'record_click' => static function ($type, $id, $client) {
                self::record_search_trending_click($type, $id, $client);
            },
            'normalize_context' => static function ($context) {
                return self::normalize_search_context($context);
            },
            'discovery_groups' => static function ($limit, $context) {
                return self::build_search_discovery_groups($limit, $context);
            },
            'query_suggestions' => static function ($context) {
                return self::search_query_suggestions($context);
            },
            'search_groups' => static function ($query, $limit, $context) {
                return self::build_search_groups($query, $limit, $context);
            },
            'build_suggestion' => static function ($query, $groups) {
                return self::search_build_query_suggestion($query, $groups);
            },
            'parse_intent' => static function ($query, $limit, $context) {
                return self::search_parse_intent($query, $limit, $context);
            },
        ]);
    }
    ...
    public static function render_dashboard_page()
    {
        ...
        $counts = [
            'db_matches' => self::table_exists($matches_table) ? (int) $wpdb->get_var("SELECT COUNT(*) FROM {$matches_table}") : 0,
            'db_games' => self::table_exists($games_table) ? (int) $wpdb->get_var("SELECT COUNT(*) FROM {$games_table}") : 0,
            'db_sets' => self::table_exists($sets_table) ? (int) $wpdb->get_var("SELECT COUNT(*) FROM {$sets_table}") : 0,
        ];
